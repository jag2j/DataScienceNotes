...
$$
  \textbf H = \textbf X(\textbf X' \textbf X) \textbf X'
$$
...
$$
Note that the $SS_T$ can also be calculated with the formula used in simple linear regression (see p. 86 of the textbook).
...
## F test to see if any coefficient is nonzero
...
Read the book starting at p. 89.  Seriously, there's a lot of detail that we skip over here.
...
We won't go into which distributions hold (see p. 90), but we can test the F statistic as if the statistic follows a $F_{r,n-p}$ distribution.  If your level of significance is $\alpha$, check for $F_0 > F_{\alpha,r,n-p}$ and if so, reject the null hypothesis.
...
$$
  \hat\beta_j - t_{A/2,n-p}\sqrt{\hat\sigma^2C_{jj}} \le \beta_j \le \hat\beta_j + t_{A/2,n-p}\sqrt{\hat\sigma^2C_{jj}}
$$
